    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-surface-600 mt-1">{exames.length} pedidos</p>
        </div>
        <Link href="/app/exames/novo" className="btn-primary"><Plus className="w-4 h-4" />Novo pedido</Link>
      </div>
      {loading ? (
            ...
            <div key={exame.id} className="card-hover">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  {exame.indicacao_clinica && <p className="text-xs text-surface-500 mt-1">Indicacao: {exame.indicacao_clinica}</p>}
                </div>
              </div>
            </div>
          ))}
